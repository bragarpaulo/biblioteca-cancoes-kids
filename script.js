/* ============================================================
   IBPROF — Biblioteca de Canções Kids
...
(function () {
  ...
  function captureUTMs() {
    ...
    if (hasFresh) {
      try { sessionStorage.setItem('bck_utms', JSON.stringify(fresh)); } catch (_) {}
      return fresh;
    }
    try {
      const stored = sessionStorage.getItem('bck_utms');
      if (stored) return JSON.parse(stored);
      sessionStorage.setItem('bck_utms', JSON.stringify(fresh));
    } catch (_) {}
    return fresh;
  }
  ...
  function captureClickIds() {
    ...
    CLICK_ID_KEYS.forEach((k) => {
      let v = params.get(k);
      if (!v) {
        try { v = sessionStorage.getItem('bck_' + k) || ''; } catch (_) { v = ''; }
      }
      if (v) {
        out[k] = v;
        try { sessionStorage.setItem('bck_' + k, v); } catch (_) {}
      }
    });
    return out;
  }
  ...
  window.BCK_UTMS = UTMS;
  window.BCK_CLICK_IDS = CLICK_IDS;
  window.BCK_CUSTOM_DATA = function () {
    ...
    try {
      if (window.fbq) {
        window.fbq('track', 'InitiateCheckout', Object.assign(
          { content_name: 'biblioteca-cancoes-kids', content_category: 'educacao-musical-infantil' },
          UTMS, CLICK_IDS
        ));
      }
    } catch (_) {}
